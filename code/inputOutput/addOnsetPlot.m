function [figH,indexSubplot] = addOnsetPlot(timeV, dataRaw, dataFiltered, ...
    signalWindowInterval, noiseModelWindows, onsetWindows, timeReference, ...
    onsetColor, dataLabel, figH, subPlotPanel, indexSubplot, labelOffset)
...
if(isempty(onsetWindows)==0)
    ...
    for k=1:1:size(onsetWindows,1)   
        ...
        if(k==1)
            ...
            vtt = yLimMax - (yLimMax-yLimMin)*(0.15*(labelOffset+1));
            plot([t0;tt],[vVal;vtt],'--','Color',onsetColor);
            hold on;       
            
            if(isempty(timeReference)==0)
                dt = t0-timeReference;                
                text(tt,vtt,...
                    sprintf('(%1.3f, %1.3f)\n%s%1.3f%s',t0,vVal,...
                                '$$\Delta t: ',dt,'$$'),...
