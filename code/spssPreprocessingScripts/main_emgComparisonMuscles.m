...
conditionsToCompare(1).carDirection='Right';

conditionsToCompare(2).condition='seatBack';
conditionsToCompare(2).carDirection='Right';
...
switch flag_calculateMeanAndExtraColums
    ...
    case 1
        spssTable.columnLabels ={'id','sex'};
        positionOnsetLabels=2;
        positionAmplitudeLabels=2+(numberOfMuscles*numberOfConditions);
        for indexCondition=1:1:numberOfConditions
            for indexMuscle=firstMuscleBiopacIndex:1:lastMuscleBiopacIndex
                ...
                nameOnsetLabels = ['Onset',nameMuscle, nameCondition , nameCarDirection];
                nameAmplitudeLabels = ['Amplitude',nameMuscle, nameCondition , nameCarDirection];
...
if flag_calculateMeanAndExtraColums == 1
       spssRow = zeros(numberOfParticipants,length(spssTable.columnLabels));

    for i=participantFirst:1:participantLast
            participantData = getParticipantDataFebruary2023(i);
            [val(i),idSex(i)] = max(contains(sexNames,participantData.sex));
            spssRow(i,1) = i;
    end
        spssRow(:,2) = idSex;
    for indexCondition=1:1:numberOfConditions
        for indexMuscle=firstMuscleBiopacIndex:1:lastMuscleBiopacIndex
            positionOnsetLabels=positionOnsetLabels+1;
            positionAmplitudeLabels=positionAmplitudeLabels+1;
            spssRow(:,positionOnsetLabels)=outputValidData(indexCondition,indexMuscle).validDataOnsetTimes;
            spssRow(:,positionAmplitudeLabels)=outputValidData(indexCondition,indexMuscle).validDataAmplitudes;
        end
    end
